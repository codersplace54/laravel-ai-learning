...
<head>
    <meta charset="UTF-8">
    <title>Import Legal Metrology Applications (Excel)</title>
...
<body class="bg-light">
    ...
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3 mb-0">Import Legal Metrology Applications (Excel)</h1>
            ...
            @if (session('skipped_count', 0) > 0)
                @php
                    $reason_labels = [
                        ...
                        'service_not_found' => 'Service not found / not mapped',
                                        ...
                                        <ul class="mb-0">
                                            @foreach ($group['rows'] as $row)
                                                <li class="mb-1">
                                                    ...
                                                    NOC Master ID: {{ $row['noc_master_id'] ?? 'N/A' }},
                                                    Old User ID: {{ $row['old_user_id'] ?? 'N/A' }},
                                                    Reason: {{ $row['reason'] ?? 'N/A' }}
                                                    @if(isset($row['raw_status']))
                                                        <br><small class="text-muted">Raw Status: {{ $row['raw_status'] }}</small>
                                                    @endif
                                                </li>
                                            @endforeach
                                        </ul>
            ...
            @if (session('assignment_skipped_count', 0) > 0)
                @php
                    $assignment_reason_labels = [
                        ...
                        'no_steps_found_for_service' => 'No steps found for service',
                        'unknown' => 'Unknown',
                    ];
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('admin.import.legal_metrology') }}" method="POST"
                    ...
                    <div class="mb-3">
                        ...
                        <small class="text-muted">
                            Columns: #, NOC_details_ID, vid, NOC_master_ID, title, old_user_id, application_date, final_fee, applicationId, PaymentStatus, application_status, NOC_type, NOC_Certificate, NOC_Certificate_Number, NOC_application_date, NOC_expiry_date, NOC_generation_date, NOC_mode
                        </small>
                    </div>

                    <button type="submit" class="btn btn-primary">
                        Import Legal Metrology Applications
                    </button>
                </form>
            </div>
        </div>
